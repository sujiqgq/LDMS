...
var
  frmLD4Q141: TfrmLD4Q141;

implementation

uses ZZFunc, ZZMenu, ZZComm, MdmsFunc, LD4Q14;

{$R *.DFM}

procedure TfrmLD4Q141.QuickRepNeedData(Sender: TObject;
  var MoreData: Boolean);
begin
   inherited;

   with frmLD4Q14.grdMaster do
   begin
      // 자료가 없을경우의 처리
      if Rows = 0 then
      begin
...
procedure TfrmLD4Q141.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
   inherited;
   QRL_date.Caption := copy(frmLD4Q14.EdtBDate.Text,1,4) + '년'  +
                       copy(frmLD4Q14.EdtBDate.Text,5,2) + '월'  +
                       copy(frmLD4Q14.EdtBDate.Text,7,2) + '일';
   UV_iCount := 1;
   if ((frmLD4Q14.grdMaster.Rows mod 100) = 0) then
      sRec := Round(frmLD4Q14.grdMaster.Rows div 100)
   else
   begin
      sRec := Round(frmLD4Q14.grdMaster.Rows div 100) + 1;
   end;
   cRow := 0; iRow := 0;
end;
