...
type
  TfrmLD4Q16 = class(TfrmSingle)
    ...
    qryTkgum: TQuery;
    GroupBox1: TGroupBox;
    RBtn_S101: TRadioButton;
    RBtn_S102: TRadioButton;
    RBtn_S101S102: TRadioButton;
    qryProfile: TQuery;
    Btn_NamePrint: TBitBtn;
    RBtn_2: TRadioButton;
    RBtn_1: TRadioButton;
    RBtn_S104: TRadioButton;
    Label3: TLabel;
  ...
  private
    ...
    UV_vNo_rack, UV_vName, UV_vJumin, UV_vNum_samp, UV_vBUN, UV_vHM, UV_vHM1, UV_vdat_bunju, UV_vdat_gmgn, UV_vnum_jumin, UV_vdesc_dc, UV_vsex,
    UV_vNo : Variant;
...
var
  frmLD4Q16: TfrmLD4Q16;

implementation

uses ZZFunc, ZZMenu, ZZComm, MdmsFunc, LD4Q161, LD4Q162, LD4Q163;

{$R *.DFM}

procedure TfrmLD4Q16.UP_KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TfrmLD4Q16.MouseWheelHandler(var Message: TMessage);  //수정
begin
...
procedure TfrmLD4Q16.UP_VarMemSet(iLength : Integer);
begin
...
function TfrmLD4Q16.UF_Condition : Boolean;
begin
   Result := True;
...
procedure TfrmLD4Q16.FormCreate(Sender: TObject);
begin
   inherited;
   UP_VarMemSet(0);
...
procedure TfrmLD4Q16.grdMasterCellLoaded(Sender: TObject; DataCol,
  DataRow: Integer; var Value: Variant);
begin
   inherited;
   ...
   case DataCol of
      ...
      9 : Value := UV_vdat_gmgn[DataRow-1];
     10 : Value := UV_vNum_jumin[DataRow-1];
...
procedure TfrmLD4Q16.btnQueryClick(Sender: TObject);
...
begin
   ...
   if RBtn_S104.checked then
   begin
   grdMaster.Col[7].Heading := '알레르기 107종';
   grdMaster.Col[8].Heading := '';
   end
   else if RBtn_S101.checked then
   begin
   grdMaster.Col[7].Heading := '알레르기 62종(흡입성)';
   grdMaster.Col[8].Heading := '';
   end
   else if RBtn_S102.checked then
   begin
   grdMaster.Col[7].Heading := '알레르기 62종(식이성)';
   grdMaster.Col[8].Heading := '';
   end
   else if RBtn_S101S102.checked then
   begin
   grdMaster.Col[7].Heading := '알레르기 62종(흡입성)';
   grdMaster.Col[8].Heading := '알레르기 62종(식이성)';
   end;
   ...
   begin
      ...
      begin
         ...
         Begin
            ...
            if (RBtn_S104.Checked = True) and (pos('S104',sHul_List) > 0) Then
            begin
               ...
               // 주민번호(생년월일) -> MM-DD-성별-센터-YY로 나열 ..20170922..수지
                    if qryBunju.FieldByName('cod_Jisa').AsString = '15' then sjisa := '0'
               else if qryBunju.FieldByName('cod_Jisa').AsString = '12' then sjisa := '2'
               else if qryBunju.FieldByName('cod_Jisa').AsString = '11' then sjisa := '1'
               else if qryBunju.FieldByName('cod_Jisa').AsString = '43' then sjisa := '4'
               else if qryBunju.FieldByName('cod_Jisa').AsString = '71' then sjisa := '7'
               else if qryBunju.FieldByName('cod_Jisa').AsString = '61' then sjisa := '6'
               else if qryBunju.FieldByName('cod_Jisa').AsString = '51' then sjisa := '5';
               case StrToInt(copy(qryBunju.FieldByName('num_jumin').AsString,7,1)) of
                  1,3,5,7 : UV_vSex[Index] := Copy(qryBunju.FieldByName('num_jumin').AsString, 3, 4) + 'M' + sjisa + Copy(qryBunju.FieldByName('num_jumin').AsString, 1, 2);
                  2,4,6,8 : UV_vSex[Index] := Copy(qryBunju.FieldByName('num_jumin').AsString, 3, 4) + 'M' + sjisa + Copy(qryBunju.FieldByName('num_jumin').AsString, 1, 2);
               end;

               if pos('S104', sHul_List) > 0 then
               begin
                    UV_vHM[Index] := '';
                    Sum_vHM := Sum_vHM + 1;
               end
            ...
            else if (RBtn_S101.Checked = True) and (pos('S101',sHul_List) > 0) Then
            begin
               UP_VarMemSet(Index+1);
               UV_vNo[Index] := Index+1;
               UV_vNo_rack[Index]         :=qryBunju.FieldByName('desc_rackno').AsString;
            ...
            else if (RBtn_S102.Checked = True) and (pos('S102',sHul_List) > 0) Then
            begin
               UP_VarMemSet(Index+1);
               UV_vNo[Index] := Index+1;
               UV_vNo_rack[Index]  :=qryBunju.FieldByName('desc_rackno').AsString;
            ...
            else if (RBtn_S101S102.Checked = True) and ((pos('S101',sHul_List) > 0) or (pos('S102',sHul_List) > 0))  Then
            begin
               UP_VarMemSet(Index+1);
               UV_vNo[Index] := Index+1;
               UV_vNo_rack[Index]  :=qryBunju.FieldByName('desc_rackno').AsString;
...
procedure TfrmLD4Q16.UP_Change(Sender: TObject);
var sName : String;
begin
  inherited;
...
procedure TfrmLD4Q16.UP_Click(Sender: TObject);
...
procedure TfrmLD4Q16.btnPrintClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmLD4Q16.QRCompositeReportAddReports(Sender: TObject);
begin
  inherited;
  frmLD4Q161 := TfrmLD4Q161.Create(Self);

  with QRCompositeReport do
  begin
    Reports.Add(frmLD4Q161.QuickRep);
  end;
end;

function TfrmLD4Q16.UF_hangmokList : String;
var
...
function TfrmLD4Q16.UF_No_Hangmok(sDate, sGubun : String; iYh : Integer): String;
begin
   Result := '';
...
procedure TfrmLD4Q16.FormActivate(Sender: TObject);
begin
  inherited;
   mskFrom.Text := '0001';
   mskTo.Text := '9999';
...
procedure TfrmLD4Q16.Btn_NamePrintClick(Sender: TObject);
begin
  inherited;
  if RBtn_1.Checked then
  begin
   frmLD4Q162 := TfrmLD4Q162.Create(Self);
   if RBtn_preveiw.Checked then frmLD4Q162.QuickRep.Preview
   else                         frmLD4Q162.QuickRep.Print;
  end
  else
   begin
   frmLD4Q163 := TfrmLD4Q163.Create(Self);
   if RBtn_preveiw.Checked then frmLD4Q163.QuickRep.Preview
   else                         frmLD4Q163.QuickRep.Print;
  end;
end;
