...
// 수정일자      : 2009.12.06
// 수정자        : 송재호
// 수정내용      : 혈액형 주민번호로 끌어오게 변경
//==============================================================================
// 수정일자      : 2011.01.26
// 수정자        : 송재호
// 수정내용      : H024가 무조건 RH+가 들어가지 않고 이전결과 끌어오도록..
//==============================================================================
// 수정일자      : 2011.01.26
// 수정자        : 송재호
// 수정내용      : H024가 무조건 RH+가 들어가지 않고 이전결과 끌어오도록..
// 참고사항      : 원래대로 H024가 무조건 RH+로 들어가도록 수정(예비요청)
//==============================================================================
// 수정일자      : 2012.02.06
// 수정자        : 유동구
...
procedure TfrmLD1I011.FormCreate(Sender: TObject);
var  sJisa : string;
begin
   inherited;
...
procedure TfrmLD1I011.UpClick(Sender: TObject);
...
begin
   ...
   begin
      ...
      begin
         sWhere := sWhere + ' AND A.cod_jisa in (''15'', ''34'')';
      end;
      sWhere := sWhere + ' AND A.dat_bunju = ''' + mskDate.Text + '''';

      sWhere := sWhere + ' AND A.gubn_part = ''02''';
      sWhere := sWhere + ' AND A.num_id <> ''1500016357''';
      ...
      begin
         GP_query_log(qryGlkwa, 'LD1I011', 'Q', 'Y');
...
//[2012.03.26]H018~H022까지 데이터 값 삭제...
//---------------------------------------------------------------------------
            if Chk_Del.Checked then
            begin
               if Pos('H018', sTcode) > 0 then UV_fGulkwa := UF_Gulkwa_Diff(UV_fGulkwa, 103, 108, '');
               if Pos('H019', sTcode) > 0 then UV_fGulkwa := UF_Gulkwa_Diff(UV_fGulkwa, 109, 114, '');
               if Pos('H020', sTcode) > 0 then UV_fGulkwa := UF_Gulkwa_Diff(UV_fGulkwa, 115, 120, '');
               if Pos('H021', sTcode) > 0 then UV_fGulkwa := UF_Gulkwa_Diff(UV_fGulkwa, 121, 126, '');
               if Pos('H022', sTcode) > 0 then UV_fGulkwa := UF_Gulkwa_Diff(UV_fGulkwa, 127, 132, '');
            end;

//[2012.03.07]액형 실제 검사자만 일괄정리 가능하게...
//---------------------------------------------------------------------------
               ...
               begin
                  ...
                  begin
                     ...
                     begin
                        ...
                        {if ckbH024.Checked then
                        begin
                           if (Pos('H024', sTcode) > 0) AND (Trim(Copy(qryPrev.FieldByName('desc_glkwa1').AsString, 139, 6)) <> '') then
                              UV_fGulkwa := UF_Gulkwa(UV_fGulkwa, 139, 144, Copy(qryPrev.FieldByName('desc_glkwa1').AsString, 139, 6));
                        end;}
                        Break;
                     end;
                     qryPrev.Next;
...
function TfrmLD1I011.UF_CodeCheck(sCode, sTcode : String) : Boolean;
var i, ii : Integer;
begin
   Result := False;
   ii := 1;
...
function TfrmLD1I011.UF_Code(sHul, sCancer, sJangbi, sChuga : String) : String;
var i, ii : Integer;
    sTcode : String;
begin
   Result := '';
...
function  TfrmLD1I011.UF_No_Hangmok(sDate, sGubun : String; iYh : Integer): String;
begin
   Result := '';
...
function  TfrmLD1I011.UF_Jae_Hangmok(sJisa, sJumin, sGubun : String; iYh : Integer): String;
begin
   Result := '';
   with qryJaegumhm do
   begin
...
function  TfrmLD1I011.UF_Gulkwa(sGulkwa : String ; iStart, iEnd : Integer ; sGul : String): String;
begin
   Result := sGulkwa;
...
function  TfrmLD1I011.UF_Gulkwa_Diff(sGulkwa : String ; iStart, iEnd : Integer ; sGul : String): String;
begin
   Result := sGulkwa;
   if Trim(Copy(sGulkwa, iStart, iEnd-iStart)) = '0' then
      sGulkwa := GF_RPad(Copy(sGulkwa, 1, iStart-1), iStart-1, ' ')
               + GF_RPad(sGul, iEnd - iStart + 1, ' ')
               + Copy(sGulkwa, iEnd+1, Length(sGulkwa));
   Result := sGulkwa;
...
procedure TfrmLD1I011.btnCancelClick(Sender: TObject);
begin
  inherited;
   close;
end;

function  TfrmLD1I011.UF_Tk_Hangmok(sDate : String; iYh : Integer; iJumin : String): String;
Var
   i, J, K : Integer;
begin
   Result := '';
