...
var
  frmLD1P12: TfrmLD1P12;

implementation

uses ZZFunc, ZZMenu, ZZComm, MdmsFunc, LDAI02, LD1P121, LD1P122;

{$R *.DFM}

procedure TfrmLD1P12.UP_KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TfrmLD1P12.UP_Change(Sender: TObject);
var sName : String;
begin
  inherited;
...
procedure TfrmLD1P12.UP_Click(Sender: TObject);
...
procedure TfrmLD1P12.btnPreviewClick(Sender: TObject);
begin
  inherited;
  ...
  begin
    frmLD1P121 := TfrmLD1P121.Create(Self);

     // SQL문을 저장
     UV_sBasicSQL := frmLD1P121.qryGumgin.SQL.Text;

     if UF_Query then
        frmLD1P121.QuickRep.Preview
     else
        ShowMessage('자료가 존재하지 않습니다');
  end
  else if Chk_New.Checked = True then
  begin
    frmLD1P122 := TfrmLD1P122.Create(Self);

     // SQL문을 저장
     UV_sBasicSQL := frmLD1P122.qryGumgin.SQL.Text;

     if UF_Query2 then
        frmLD1P122.QuickRep.Preview
     else
        ShowMessage('자료가 존재하지 않습니다');
  end;
...
function  TfrmLD1P12.UF_Query : Boolean;
var sWhere : String;
begin
   ...
   begin
      ...
      GP_query_log(frmLD1P121.qryGumgin, 'LD1I12', 'Q', 'N');
      Active := True;

      if frmLD1P121.qryGumgin.IsEmpty = False then
      begin
         if rbAll.Checked then
         begin
            frmLD1P121.QuickRep.PrinterSettings.FirstPage := 0;
            frmLD1P121.QuickRep.PrinterSettings.LastPage  := 0;
         end
         else if rbPart.Checked then
         begin
            frmLD1P121.QuickRep.PrinterSettings.FirstPage := StrToInt(valFrom.Text);
            frmLD1P121.QuickRep.PrinterSettings.LastPage  := StrToInt(valTo.Text);
         end;
         Result := True;
      end
      else
...
function TfrmLD1P12.UF_Condition : Boolean;
begin
   Result := True;
...
procedure TfrmLD1P12.FormCreate(Sender: TObject);
var  sJisa : string;
begin
  inherited;
   ...
   msksDate.Text := frmLDAI02.mskDate.Text;
   mskeDate.Text := frmLDAI02.mskDate.Text;
   CmbJIsa.Text := frmLDAI02.grdMaster.Cell[1,frmLDAI02.grdMaster.CurrentDataRow];
   GP_ComboSeek(cmbJisa);
   edtsName.Text := frmLDAI02.grdMaster.Cell[3,frmLDAI02.grdMaster.CurrentDataRow];
   edteName.Text := frmLDAI02.grdMaster.Cell[3,frmLDAI02.grdMaster.CurrentDataRow];
   Edts_No.Text  := frmLDAI02.edtDESC_BUWI.Text;
   Edte_No.Text  := frmLDAI02.edtDESC_BUWI.Text;
   gubun.Text    := frmLDAI02.grdMaster.Cell[6,frmLDAI02.grdMaster.CurrentDataRow];
   GP_ComboSeek(gubun);
   R_Date.Text   := frmLDAI02.mskDat_Result.Text;
...
procedure TfrmLD1P12.btnPrintClick(Sender: TObject);
begin
  inherited;
  if Chk_New.Checked = False then
  begin
...
procedure TfrmLD1P12.btnCloseClick(Sender: TObject);
begin
  inherited;
  close;
end;

procedure TfrmLD1P12.Ck_DoctorClick(Sender: TObject);
begin
  inherited;
...
function TfrmLD1P12.UF_Query2: Boolean;
var sWhere : String;
begin
   ...
   begin
      ...
      SQL.Clear;
      SQL.Add(UV_sBasicSQL + sWhere);
      GP_query_log(frmLD1P122.qryGumgin, 'LD1I12', 'Q', 'N');
      Active := True;

      if frmLD1P122.qryGumgin.IsEmpty = False then
      begin
         if rbAll.Checked then
         begin
            frmLD1P122.QuickRep.PrinterSettings.FirstPage := 0;
            frmLD1P122.QuickRep.PrinterSettings.LastPage  := 0;
         end
         else if rbPart.Checked then
         begin
            frmLD1P122.QuickRep.PrinterSettings.FirstPage := StrToInt(valFrom.Text);
            frmLD1P122.QuickRep.PrinterSettings.LastPage  := StrToInt(valTo.Text);
         end;
         Result := True;
      end
      else
