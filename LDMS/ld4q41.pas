...
type
  ...
  private
    ...
    UV_vNo, UV_vNum_samp, UV_vSBun, UV_vName, UV_vage, UV_vC049 : Variant;
...
procedure TfrmLD4Q41.UP_VarMemSet(iLength : Integer);
begin
...
function TfrmLD4Q41.UF_Condition : Boolean;
begin
   Result := True;
...
procedure TfrmLD4Q41.FormCreate(Sender: TObject);
begin
   inherited;
   UP_VarMemSet(0);
...
procedure TfrmLD4Q41.grdMasterCellLoaded(Sender: TObject; DataCol,
  DataRow: Integer; var Value: Variant);
begin
   inherited;
   // 자룔를 화면에 조회
   case DataCol of
      ...
      6 : Value := UV_vC049[DataRow-1];
   end;
end;

procedure TfrmLD4Q41.btnQueryClick(Sender: TObject);
...
begin
   ...
   begin
      // SQL문 생성
      Close;
      sSelect := 'Select  a.cod_jisa, a.num_id, a.dat_gmgn, a. num_jumin, a.num_samp, a.desc_name, a.cod_hul, a.cod_chuga, b.desc_glkwa1, b.num_bunju';
      sSelect := sSelect + ' From Gumgin a with(nolock) left outer join gulkwa b with(nolock) on a.cod_jisa = b.cod_jisa and a.num_id = b.num_id and a.dat_gmgn = b.dat_gmgn ';
      ...
      begin
         ...
         begin
            ...
            GP_JuminToAge(qryGulkwa.FieldByName('num_jumin').AsString, qryGulkwa.FieldByName('dat_gmgn').AsString, sMan, iAge);
               if (sMan='F') and (pos('C049', sHul_List) > 0)and (trim(copy(qryGulkwa.FieldByName('desc_glkwa1').AsString,7,6)) <> '')  then
               begin
                  ...
                  UV_vage[index]       := iAge;
                  UV_vC049[index]      := trim(copy(qryGulkwa.FieldByName('desc_glkwa1').AsString,7,6));
                  Inc(index);
                  Gride.Progress := I;
               end;
            end;
...
procedure TfrmLD4Q41.UP_Click(Sender: TObject);
begin
   inherited;
...
procedure TfrmLD4Q41.FormActivate(Sender: TObject);
begin
   inherited;
   BunjuNo1.Text := '0001';
   BunjuNo2.Text := '9999';
   edtBdate.SetFocus;
    ...
    procedure TfrmLD4Q41.SBut_ExcelClick(Sender: TObject);
var
  XL,WorkBook: Variant;
  i : integer;
  ArrV3 : OleVariant;
