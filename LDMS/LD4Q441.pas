...
var
  frmLD4Q441: TfrmLD4Q441;

implementation

uses ZZFunc, ZZMenu, ZZComm, MdmsFunc, LD4Q44;

{$R *.DFM}

procedure TfrmLD4Q441.QuickRepNeedData(Sender: TObject;
  var MoreData: Boolean);
begin
   inherited;

   with frmLD4Q44.grdMaster do
   begin
      // 자료가 없을경우의 처리
      if Rows = 0 then
      begin
...
procedure TfrmLD4Q441.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
   inherited;
   QRL_date.Caption := copy(frmLD4Q44.EdtBDate.Text,1,4) + '년'  +
                       copy(frmLD4Q44.EdtBDate.Text,5,2) + '월'  +
                       copy(frmLD4Q44.EdtBDate.Text,7,2) + '일';
   UV_iCount := 1;
   if ((frmLD4Q44.grdMaster.Rows mod 100) = 0) then
      sRec := Round(frmLD4Q44.grdMaster.Rows div 100)
   else
   begin
      sRec := Round(frmLD4Q44.grdMaster.Rows div 100) + 1;
   end;
   cRow := 0; iRow := 0;
end;
