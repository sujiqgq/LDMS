...
var
  frmLD6Q021: TfrmLD6Q021;

implementation

uses ZZFunc, ZZMenu, ZZComm, MdmsFunc, LD6Q02;

{$R *.DFM}

procedure TfrmLD6Q021.QuickRepNeedData(Sender: TObject;
  var MoreData: Boolean);
begin
   inherited;

         QRLabel1.caption :=frmLD6Q02.edt_sokun.Text + ' 혈액소견 리스트';



   with frmLD6Q02.grdMaster do
   begin
      // 자료가 없을경우의 처리
      if Rows = 0 then
      begin
      ...
      begin
         case StrToInt(copy(Cell[5,(iRow * 50) + cRow],8,1)) of
      ...
      begin
         case StrToInt(copy(Cell[5,(iRow * 50) + cRow + 50],8,1)) of
...
procedure TfrmLD6Q021.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
   inherited;
   QRL_date.Caption := copy(frmLD6Q02.mskDate.Text,1,4) + '년'  +
                       copy(frmLD6Q02.mskDate.Text,5,2) + '월'  +
                       copy(frmLD6Q02.mskDate.Text,7,2) + '일';
   UV_iCount := 1;
   if ((frmLD6Q02.grdMaster.Rows mod 100) = 0) then
      sRec := Round(frmLD6Q02.grdMaster.Rows div 100)
   else
   begin
      sRec := Round(frmLD6Q02.grdMaster.Rows div 100) + 1;
   end;
   cRow := 0; iRow := 0;
end;
